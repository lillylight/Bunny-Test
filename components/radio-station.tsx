...
export function RadioStation() {
  ...
  useEffect(() => {
    if (isPlaying) {
      // Check if we should play the station intro (once per day)
      const today = new Date().toDateString()
      const lastIntroDate = localStorage.getItem('busk_radio_intro_date')
      
      // Only generate welcome message if we have a real show loaded (not "Loading...")
      // AND if we haven't played the intro today
      if (!hostDialogue && currentShow.name !== "Loading..." && lastIntroDate !== today) {
        generateStationIntro()
        localStorage.setItem('busk_radio_intro_date', today)
      }
  ...
  // Generate station intro (plays once per day)
  const generateStationIntro = async () => {
    try {
      const stationIntro = `Welcome to BUSK Radio, where the future of audio entertainment comes alive! You're tuning in to ${currentShow.name}. I'm your host ${currentShow.host}, and we've got an incredible lineup for you today. Let's get started!`
      
      setHostDialogue(stationIntro)
      
      // Clear the intro message after 15 seconds
      setTimeout(() => {
        setHostDialogue("")
      }, 15000)
      
      // Use TTS for station intro
      const response = await fetch('/api/text-to-speech', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      // Reset commentary timer
      lastCommentaryTimeRef.current = Date.now()
      recentSongsRef.current = []
    } catch (error) {
      console.error("Error generating station intro:", error)
      // Reset commentary timer even on error
      lastCommentaryTimeRef.current = Date.now()
      recentSongsRef.current = []
    }
  }
  ...
  const generateWelcomeMessage = async () => {
    try {
      const welcomeMessage = `You're now listening to ${currentShow.name}. I'm your host ${currentShow.host}, and we've got some amazing content lined up for you. Stay tuned!`
  ...
  useEffect(() => {
    ...
    if (currentShow.name !== lastShowNameRef.current && currentShow.name !== "Loading...") {
      // Stop advertising for previous show
      if (lastShowNameRef.current) {
        advertisingService.stopShow()
      }
      
      // Start advertising for new show
      const showDuration = (currentShow as any).duration || 60
      advertisingService.startShow(currentShow.name, showDuration)
    ...
    return () => {
      ...
      // Stop advertising when component unmounts
      advertisingService.stopShow()
    }
